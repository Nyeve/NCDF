...
# Shiny Dashboard: NCDF Allocation Trends
...
# NCDF_long is pre-loaded with columns: County, Constituency, Year, Allocation
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sliderInput(
      ...
      value = c(
        min(NCDF_long$Year, na.rm = TRUE),
        max(NCDF_long$Year, na.rm = TRUE)
      ),
      step = 1,
      sep = ""
    ),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "comparison", fluidRow(
        box(
      ...
      tabItem(tabName = "growth", fluidRow(
        box(
          title = "Growth Rate of Allocations",
          status = "warning",
          solidHeader = TRUE,
          width = 12,
      ...
      tabItem(tabName = "top", fluidRow(
        box(
          title = "Top Constituency per Year",
          status = "success",
          solidHeader = TRUE,
          width = 12,
          DTOutput("topPerformers")
        )
      )),
      tabItem(tabName = "cross", fluidRow(
        box(
          title = "Cross-County Stacked Comparison",
          status = "danger",
          solidHeader = TRUE,
          width = 12,
          plotOutput("facetStackedBar")
        )
      ))
    )
  )
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive({
    req(input$constituency)  # ensures it's not NULL
    NCDF_long %>%
      filter(
        County == input$county,
  ...
  output$growthBox <- renderValueBox({
    df <- filteredData()
    growth <- ifelse(nrow(df) > 1, (last(df$Allocation) - first(df$Allocation)) / first(df$Allocation) * 100, NA)
    valueBox(
      ifelse(is.na(growth), "N/A", paste0(round(growth, 2), "%")),
      "Growth Rate",
      icon = icon("chart-line"),
      color = "purple"
    )
  ...
  output$barChart <- renderPlot({
    ...
    if (input$scaleType == "millions") {
      ...
      df <- df %>% mutate(Display = Total_Allocation / sum(Total_Allocation) *
                            100)
    ...
    ggplot(df, aes(
      x = reorder(Constituency, -Display),
      y = Display,
      fill = Constituency
    )) +
      geom_bar(stat = "identity") +
      geom_text(aes(label = label_fun(Display)),
                vjust = -0.5,
                size = 3) +
  ...
  output$growthChart <- renderPlot({
    df <- NCDF_long %>%
      ...
      group_by(Constituency) %>%
      summarise(
        GrowthRate = (last(Allocation) - first(Allocation)) / first(Allocation) * 100,
        .groups = "drop"
      )
    
    ggplot(df, aes(
      x = reorder(Constituency, GrowthRate),
      y = GrowthRate,
      fill = Constituency
    )) +
      geom_bar(stat = "identity") +
      ...
      coord_flip() +
      labs(
        title = paste("Growth Rate of Allocations (2014–2026) in", input$county),
        y = "Growth Rate (%)"
      ) +
      theme_minimal()
  })
  ...
  output$topPerformers <- renderDT({
    df <- NCDF_long %>%
      filter(County == input$county,
             ...
             Year <= input$yearRange[2]) %>%
      group_by(Year) %>%
      slice_max(order_by = Allocation,
                n = 1,
                with_ties = FALSE)
    
    datatable(df[, c("Year", "County", "Constituency", "Allocation")], options = list(pageLength = 10, autoWidth = TRUE))
  })
  ...
  output$facetStackedBar <- renderPlot({
    ...
    ggplot(df, aes(
      ...
      labs(title = "Yearly Allocations by Constituency", y = "Allocation (Million KES)") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
  })
}
